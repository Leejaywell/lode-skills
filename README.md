...
| # | 命令（= 技能名） | 干什么 | 产出 |
|---|---|---|---|
| 1 | `/lode-spec` | 把模糊想法**逼问**成可开发需求；开局把现状图备好（改现有代码时走 delta） | `product-spec.md` + `system-map.md` |
...
> 前置：[Claude Code](https://claude.com/claude-code)。**推荐插件装（方式 1）**——装一次,之后任意项目直接 `/lode-spec`,其余全自动。**脚本装（方式 2）** 是插件系统不可用时的兜底。
...
不用 clone，一行装进 `~/.claude/`：
```bash
curl -fsSL https://raw.githubusercontent.com/Leejaywell/lode-skills/main/install.sh | bash
```
> 想先看再跑：`curl -fsSL <同一 URL> -o /tmp/lode.sh && bash /tmp/lode.sh`。`CLAUDE_HOME=/path` 可改装机目录。

装进 `skills/lode-*`、`agents/lode-*`（命令为**裸** `/lode-spec`…），门禁脚本放到 `~/.claude/lode-hooks/`。`CLAUDE.md`/`verify.sh` 同样由流程自动铺。**唯一手动一步**（因为没有插件来挂门禁）：把 `hooks/settings.json` 的 `hooks` 块合并进项目 `.claude/settings.json`（脚本走 `$CLAUDE_PROJECT_DIR/hooks/`,先 `cp -R ~/.claude/lode-hooks/. ./hooks && chmod +x ./hooks/*.sh`）。嫌烦就用方式 1。
...
/lode-drive 把 <目标> 做完
```
`lode-drive` 自己判断**从零新建 / 改现有代码**与**单人/团队**，拆里程碑→Face，逐个走四步审计+回归，维护进度账本（崩了能续、跑完能审计），偏离就重规划、卡住就熔断。你只在**审 PR**和**接熔断**时出现。
