...
装进 `skills/lode-*`、`agents/lode-*`（命令为**裸** `/lode-spec`…），门禁脚本放到 `~/.claude/lode-hooks/`、源资产（`CLAUDE.md` + 模板）放到 `~/.claude/lodestar/`（供 spec/build 自动铺设取用）。`CLAUDE.md`/`verify.sh` 同样由流程自动铺。**唯一手动一步**（因为没有插件来挂门禁）：把 `hooks/settings.json` 的 `hooks` 块合并进项目 `.claude/settings.json`（脚本走 `$CLAUDE_PROJECT_DIR/hooks/`,先 `cp -R ~/.claude/lode-hooks/. ./hooks && chmod +x ./hooks/*.sh`）。嫌烦就用方式 1。
