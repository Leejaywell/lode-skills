...
**更新**：插件——`/plugin marketplace update lodestar` 再 `/plugin update lodestar@lodestar`；脚本——重跑上面那条 `curl … | bash`（幂等覆盖，门禁不会重复挂）。更新后**新开 session** 才生效（hook 在 session 启动 / 收工时读取），你项目里已生成的 `.lode/` 产物不受影响。
...
Lodestar 全程**文档驱动**——AI 跨环节不靠记忆、靠这些文档；你也通过它们看进度、改需求。文档分两处放，按「该不该进 git」分：

**`docs/`（进 git，是你的交付物）**

| 文件 | 是什么 | 你要不要管 |
|---|---|---|
| `spec.md` | 需求的**唯一持久真相源**：当前到底要做什么。就地演进、旧条目归档、不膨胀 | ✅ 最该看的一份；需求变了就在这改（或让 AI 改） |
...
| 文件 | 是什么 | 你要不要管 |
|---|---|---|
| `system-map.md` | 代码现状的**活地图**（架构 / 约定 / 接口）。spec 建立、build 每切片更新 | 想快速搞懂「这套代码长啥样」时看 |
| `dev-plan.md` | 开发计划：切片拆分 + 每片的**验收场景** | 想知道「拆了几片、每片要满足啥」时看 |
| `changelog.md` | 每个切片做了什么（带时间戳） | 想看「这轮都改了啥」时看 |
| `design-brief.md` / `mockups/` | 设计规范 / 原型（用了 `/lode-brief`·`/lode-design` 才有） | 关心设计方向时看 |
...
| `review-passed` · `.verify-green` · `.gate-attempts` | 门禁记账：审查标记 / 验证缓存 / 熔断计数 | **完全不用管**，程序自动生成与重写 |

> **两分原则**：`docs/spec*.md` 是**持久、进 git** 的真相源；`.lode/` 全是工作态——要么周期内用完即清（plan / changelog / design），要么可重建（system-map / verify），要么纯记账（ledger / signals / 门禁缓存）。`.gitignore` 应忽略 `.lode/`、跟踪 `docs/spec*.md`——`lode-spec` / `lode-init` 开局会自动配好，你不用手动设。
>
