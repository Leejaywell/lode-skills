...
> 前置：[Claude Code](https://claude.com/claude-code)。**推荐插件装（方式 1）**——更新、卸载、门禁挂载都自动。**脚本装（方式 2）** 是插件系统不可用时的兜底。装好后在项目里跑一次 `/lode-init` 把按项目文件铺好。
...
装进 `skills/lode-*`、`agents/lode-*`（命令为**裸** `/lode-spec`、`/lode-plan`…），门禁脚本放到 `~/.claude/lode-hooks/`。脚本装没有插件的自动门禁，需在项目里手动：① `cp -R ~/.claude/lode-hooks/. ./hooks && chmod +x ./hooks/*.sh`，把 `hooks/settings.json` 的 `hooks` 块合并进 `.claude/settings.json`（脚本走 `$CLAUDE_PROJECT_DIR/hooks/`）；② 跑 `/lode-init` 铺 `CLAUDE.md` + `verify.sh`。
