...
> **Lodestar** —— 一套「产品经理 5.0」开发范式，构建在 Claude Code 的原生能力上，作为我的独立开发流程。
> 一句话：**你定星，AI 导航**。
> 内容经原视频逐字稿 + 关键帧逐项核对（见 `docs/reference/`）。
>
> **核心信念**：Prompt 正在贬值，流程设计正在升值。AI 不再是工具，而是整个开发流程的执行者。
> **人定目标，AI 跑循环**：人只做两件事——**做决定**和**验收结果**，连「定目标」都能让 AI 代笔。
>
> 它的定位是一个**产品开发教练**：不止帮你写代码，而是把整个开发过程拆成清晰流程，一步步带你走完——哪怕你刚接触 vibe coding，跟着走也能做出能用的产品。
...
1. **少造工具，多给能力** —— 给 VibCut 定义十几个剪辑小工具，结果笨得不行；推翻重来给完整通用能力，让它自己想办法，就活了。模型的聪明是**放出来的，不是设计出来的**。
...
> 实战参照 **VibCut**（视频剪辑 Agent，Mac/Electron）：0 行手写代码，全程 AI 自主，约 2h45m、~285 万 token、拆 13 个 Face；作者断续做了三四天迭代，亲自参与的时间每轮也就十几分钟。
