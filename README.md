...
| 命令（= 技能名） | 用途 |
|---|---|
| `/lode-drive` | **自主驱动器**：给一个目标，agent 拆里程碑→切片跑到底，进度账本可续可审 |
...
/lode-drive 把 <目标> 做完
```
`lode-drive` 自己判断**从零新建 / 改现有代码**与**单人/团队**，拆里程碑→切片，逐个走四步审计+回归，维护进度账本（崩了能续、跑完能审计），偏离就重规划、卡住就**熔断**（连续失败/超预算就停下交给你，不无限烧）。你只在**审 PR**和**接住熔断**时出现。
...
/lode-go      # 生成单个切片的 Go，复制发它执行 → 四步审计闭环
```
...
- 执行单个切片的三种粒度：主 Agent 直接 `lode-build` 跑完计划 / 逐个切片写 Go（最常用）/ 一条 Go 跑全部（熟练后最高效）。
