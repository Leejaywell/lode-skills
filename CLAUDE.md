...
Lodestar 为**单人 · 从 0 到 1 · 全新项目 · 做出可演示/可发布的产品**而设计，在这个盒子里最强。
- ✅ **适合**：个人/小项目从想法到 MVP、原型验证、绿地开发。
- ⚠️ **慎用 / 要补**：多人协作（子代理审查是 self-review，**不替代** peer review / PR）、棕地改造与跨模块迁移（Face 模型偏线性加功能）、长生命周期维护、安全/合规攸关系统——这些要在本流程之上叠加团队评审、迁移策略与更强的验收。
- 一句话：它擅长「把新东西做出来」，不擅长「把老系统改安全」。越界使用，先认清这点。
...
把整个目标交给自驱执行时，用 `lode-go` 生成一条 **Go** 指令；要造新能力用 `lode-skill`；规则进化用 `lode-evolve`。
