...
主 Agent **拆一个干净脑子的子 Agent**：用 `Agent` 工具调用 `lode-review` 子代理（见 `agents/lode-review.md`），把**整份相关上下文带走**（变更 diff、该切片的执行令、spec/dev-plan 片段）。子 Agent 只返回结论，**主 Agent 合并拍板**。
...
- 通过后由**主 Agent**把结论写进 `.lode/review-passed`（注明被审切片/commit，并附一行 `tree: <当前代码指纹>`——`lode-gate.sh fingerprint` 取）。门禁据此放行，且会校验该指纹与当前代码一致：**审完又改即标记失效，需重审**。
